...
def process_event(ch, method, properties, body):
    try:
        ...
        if msg_type == "uretos.lov.event.created":
            db = SessionLocal()
            try:
                existing = db.query(LOVItemReadModel).filter(LOVItemReadModel.id == data["lov_id"]).first()
                if not existing:
                    ...
                    for tr in data.get("translations", []):
                        translation = LOVTranslationReadModel(
                            id=tr["id"],
                            lov_item_id=data["lov_id"],
                            locale=tr["locale"],
                            value=tr["value"]
                        )
                        db.add(translation)
        ...
        print(f"[lov-query] Error processing event projection: {e}")
...
def start_event_consumer():
    ...
    while True:
        try:
            ...
            channel.queue_bind(queue="lov_query_events", exchange="uretos.events", routing_key="uretos.lov.event.*")
            channel.basic_consume(queue="lov_query_events", on_message_callback=process_event)
            channel.start_consuming()
        except Exception as e:
            print(f"[lov-query] Connection waiting... ({e})")
            time.sleep(3)